...
class Purchase_model extends CI_Model {
    ...
    function update_purchase_master_details($purchaseMaster,$purchaseDetailsList){
        ...
        try{
            ...
            $sql="update purchase_master set
                ...
                , roundedOff=?
                , grand_total=?
                , vehicle_fare=? where purchase_master_id=?";
            $result=$this->db->query($sql,array(
                $purchaseMaster->vendor_id
            ,$this->session->userdata('person_id')
            ,$purchaseMaster->invoice_no
            ...
            $return_array['previous record']='deleted';
    ...
    function select_all_purchase(){
        $sql="select
            purchase_master_id,vendor_id, person.mailing_name as vendor_name, person.mobile_no ,record_time,purchase_date,sum(amount+sgst+cgst+igst) as total_purchase_amount
            from (select
    ...
    function select_purchase_master_by_purchase_master_id($purchaseMasterId){
        $sql="select 
            ...
            , purchase_master.eway_bill_no
            , purchase_master.roundedOff
            , purchase_master.grand_total
            , purchase_master.vehicle_fare
