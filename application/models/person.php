...
class Person extends CI_Model {
    ...
    function get_employee_by_authentication($login_data){
        $sql="select emp_id,emp_name,user_type_id,1 as is_logged_in from employees  where employees.user_id=?  and employees.user_password=?";
        $result = $this->db->query($sql,array($login_data->userId,$login_data->userPassword));
        if($result->num_rows()>0){
            return $result->row();
        }else{
            $row['emp_id']='0';
            $row['emp_name']='not found';
            $row['user_type_id']='0';
            $row['is_logged_in']=0;
            return (object)$row;
        }
